...
import { Phone, Users, Zap, BarChart3, Tags, Mail, Inbox } from 'lucide-react'
...
export function Features() {
  ...
  // Channel icons for Unified Inbox
  const channels = [
    { icon: WhatsAppIcon, color: 'bg-green-500', name: 'WhatsApp' },
    { icon: TelegramIcon, color: 'bg-blue-500', name: 'Telegram' },
    { icon: InstagramIcon, color: 'bg-gradient-to-tr from-yellow-500 via-pink-500 to-purple-500', name: 'Instagram' },
    { icon: Mail, color: 'bg-red-500', name: 'E-posta' },
    { icon: Phone, color: 'bg-navy dark:bg-gray-700', name: 'Telefon' },
  ]
        ...
        {features.map((feature, index) => (
          <Card key={index} className="hover:shadow-lg transition-shadow duration-300">
            ...
            <CardContent>
              ...
              {/* Channel Icons for Unified Inbox */}
              {feature.showChannels && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {channels.map((channel, channelIndex) => (
                    <div 
                      key={channelIndex}
                      className="flex items-center gap-1.5 px-2 py-1 bg-gray-100 dark:bg-navy-800 rounded-full"
                      title={channel.name}
                    >
                      <div className={`w-5 h-5 ${channel.color} rounded-full flex items-center justify-center`}>
                        <channel.icon className="w-3 h-3 text-white" />
                      </div>
                      <span className="text-xs font-medium text-gray-600 dark:text-gray-300">{channel.name}</span>
                    </div>
                  ))}
                </div>
              )}
